/*******************************************************************************
* File Name: Joystick_ADC_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Joystick_ADC_IntClock_Start
********************************************************************************
*
* Summary:
...
void Joystick_ADC_IntClock_Start(void) 
{
    ...
    Joystick_ADC_IntClock_CLKEN |= Joystick_ADC_IntClock_CLKEN_MASK;
	Joystick_ADC_IntClock_CLKSTBY |= Joystick_ADC_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Joystick_ADC_IntClock_Stop
********************************************************************************
*
* Summary:
...
void Joystick_ADC_IntClock_Stop(void) 
{
    ...
    Joystick_ADC_IntClock_CLKEN &= (uint8)(~Joystick_ADC_IntClock_CLKEN_MASK);
	Joystick_ADC_IntClock_CLKSTBY &= (uint8)(~Joystick_ADC_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Joystick_ADC_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Joystick_ADC_IntClock_CLKEN & Joystick_ADC_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Joystick_ADC_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Joystick_ADC_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Joystick_ADC_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Joystick_ADC_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Joystick_ADC_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Joystick_ADC_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Joystick_ADC_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Joystick_ADC_IntClock_GetSourceRegister();
    uint16 oldDivider = Joystick_ADC_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Joystick_ADC_IntClock_CLKEN & Joystick_ADC_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Joystick_ADC_IntClock_DIV_PTR, clkDivider);
                Joystick_ADC_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Joystick_ADC_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Joystick_ADC_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Joystick_ADC_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Joystick_ADC_IntClock_DIV_PTR, clkDivider);
				Joystick_ADC_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Joystick_ADC_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Joystick_ADC_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Joystick_ADC_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Joystick_ADC_IntClock_MOD_SRC |= modeBitMask & (uint8)Joystick_ADC_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Joystick_ADC_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Joystick_ADC_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Joystick_ADC_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Joystick_ADC_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Joystick_ADC_IntClock_MOD_SRC & (uint8)(Joystick_ADC_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Joystick_ADC_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Joystick_ADC_IntClock_GetDividerRegister();
    uint8 oldSrc = Joystick_ADC_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Joystick_ADC_IntClock_MOD_SRC |= CYCLK_SSS;
        Joystick_ADC_IntClock_MOD_SRC =
            (Joystick_ADC_IntClock_MOD_SRC & (uint8)(~Joystick_ADC_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Joystick_ADC_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Joystick_ADC_IntClock_MOD_SRC & Joystick_ADC_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Joystick_ADC_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Joystick_ADC_IntClock_PHASE = clkPhase & Joystick_ADC_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Joystick_ADC_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Joystick_ADC_IntClock_PHASE & Joystick_ADC_IntClock_PHASE_MASK;
}
